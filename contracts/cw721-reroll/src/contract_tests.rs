...
use crate::msg::BaseTokenUriResponse;
use crate::{
    ContractError, Cw721Contract, ExecuteMsg, Extension, InstantiateMsg, MinterResponse, QueryMsg,
};

const MINTER: &str = "minter";
const CONTRACT_NAME: &str = "PlayerX Reroll Tokens";
const SYMBOL: &str = "PXRT";
const BASE_TOKEN_URI: &str = "uri";
...
fn setup_contract(deps: DepsMut<'_>) -> Cw721Contract<'static, Extension, Empty, Empty, Empty> {
    ...
    let msg = InstantiateMsg {
        ...
        base_token_uri: Some(String::from(BASE_TOKEN_URI)),
    };
...
fn minting() {
    ...
    assert_eq!(
        ...
        NftInfoResponse::<Extension> {
            token_uri: Some("uri/0".to_string()),
            extension: None,
        }
    );
    ...
    assert_eq!(vec!["0".to_string(), "1".to_string()], tokens.tokens);
}
...
fn burning() {
    ...
    let _ = contract
        .nft_info(deps.as_ref(), String::from("0"))
        .unwrap_err();
    ...
    assert_eq!(vec![String::from("1")], tokens.tokens);
}
...
fn transferring_nft() {
    ...
    assert_eq!(
        ...
        Response::new()
            ...
            .add_attribute("sender", String::from(MINTER))
...
fn approving_revoking() {
    ...
    let res = contract
        .execute(deps.as_mut(), mock_env(), minter, approve_msg)
        .unwrap();
    assert_eq!(
        res,
        Response::new()
...
fn approving_all_revoking_all() {
    ...
    let token_id1 = String::from("0");
    let token_id2 = String::from("1");
    ...
    let res = contract
        .execute(deps.as_mut(), mock_env(), minter, approve_all_msg)
        .unwrap();
    assert_eq!(
        res,
        Response::new()
...
fn query_tokens_by_owner() {
    ...
    // all tokens by owner
    let by_minter = vec![token_id1, token_id2, token_id3];
    let tokens = contract
        .tokens(deps.as_ref(), String::from(MINTER), None, None)
        .unwrap();
    assert_eq!(&by_minter, &tokens.tokens);

    // paginate for minter
    let tokens = contract
        .tokens(deps.as_ref(), String::from(MINTER), None, Some(1))
        .unwrap();
...
fn update_base_token_uri() {
    ...
    let minter = mock_info(MINTER, &[]);
    contract
        .execute(deps.as_mut(), mock_env(), minter, msg)
        .unwrap();
    let expected = BaseTokenUriResponse {
        base_token_uri: new_base_token_uri.to_string(),
    };
